...
# --- Word Helpers ---
def clean_export_url(url: str) -> str:
    """
    Removes query string and fragment from URL.
    Example:
    https://site.com/article?id=123#section -> https://site.com/article
    """
    if not url:
        return ""
...
def add_hyperlink(paragraph, text: str, url: str):
    ...
    Visible text is source name only.
    URL is cleaned before embedding.
    """
    clean_url = clean_export_url(url)
...
def apply_docx_styles(doc: Document):
    """
    Applies clean, simple Word styles.
    """
    styles = doc.styles
    ...
    async with httpx.AsyncClient(
        ...
        try:
            ...
            raise HTTPException(
                ...
                detail=f"Site erişimi engelledi (403/404): {str(e)}",
            )

        except Exception as e:
            raise HTTPException(
                status_code=400,
                detail=f"Bağlantı hatası: {str(e)}",
            )
...
async def analyze_news(request: AnalyzeRequest):
    ...
    if len(clean_text) < 50:
        raise HTTPException(
            status_code=400,
            detail="Anlamlı metin bulunamadı / No meaningful text found.",
        )
...
YOUR TASK:
Analyze the news, give it a score from 1-10 based on how much it affects expats, and summarize it.

CRITICAL INSTRUCTION:
You MUST write your ENTIRE response — headline, source, summary, expat_significance, category — strictly in {target_language}.
...
4. The score must be an integer from 1 to 10.
"""
...
async def export_docx(items: List[ExportItem]):
    ...
    sorted_items = sorted(
        items,
        key=lambda x: (
            x.analysis.category or "Diğer",
            -x.analysis.score,
            x.analysis.headline or "",
        ),
    )

    for category, group_iter in groupby(
        sorted_items,
        key=lambda x: x.analysis.category or "Diğer",
    ):
        ...
        for item in list(group_iter):
            ...
            source_name = (a.source or "Kaynak").strip()
            link = (a.final_url or item.input_url or "").strip()

            p.add_run("(")

            if link:
                add_hyperlink(p, source_name, link)
            else:
    ...
    return StreamingResponse(
        ...
        headers={
            "Content-Disposition": 'attachment; filename="expat_intel_export.docx"'
        },
    )
...
async def root():
    return {
        "status": "ok",
        "message": "ExpatIntel backend is running.",
    }
